from typing import TypeVar, overload

from .dimension import Dimension
from .identity import Zero, zero
from .quantity import Quantity
from .unit import DIMENSIONLESS, Unit
from .utilcollections.abc import Linear
from .variable import Variable

__all__ = ['Constant', 'constant', 'OneUnit']

T = TypeVar('T', bound=Linear)


class Constant(Quantity[T]):
    '''Constant objects are just immutable Quantity objects.'''
    @overload
    def __new__(cls, value: float, /, 
                unit: str | Unit = DIMENSIONLESS
                ) -> Constant[float]:
        '''exact constant, default unit is dimensionless.'''
    @overload
    def __new__(cls, value: T, /, 
                unit: str | Unit = DIMENSIONLESS,
                uncertainty: T | Zero = zero
                ) -> Constant[T]:
        '''set value, unit, and uncertainty.'''
    @overload
    def __new__(cls, value: T, /, 
                unit: str | Unit = DIMENSIONLESS, *,
                relative_uncertainty: T
                ) -> Constant[T]:
        '''set value, unit, and relative uncertainty.'''
    @overload
    def __new__(cls, variable: Variable[T], /, 
                unit: str | Unit = DIMENSIONLESS
                ) -> Constant[T]:
        '''set variable and unit.'''
    @classmethod
    def one(cls, unit: str | Unit) -> Constant[float]: ...  # Literal[1]
    @property
    def variable(self) -> Variable[T]: ...
    @property
    def value(self) -> T: ...
    @property
    def uncertainty(self) -> T | Zero: ...
    @property
    def relative_uncertainty(self) -> T | Zero: ...
    @property
    def unit(self) -> Unit: ...
    @property
    def dimension(self) -> Dimension: ...
    def __repr__(self) -> str: ...
    def __str__(self) -> str: ...
    def __format__(self, format_spec: str) -> str: ...
    def is_exact(self) -> bool: ...
    def is_dimensionless(self) -> bool: ...
    def copy(self) -> Constant: ...

    def to(self, new_unit: str | Unit, *,
           assertDimension=True) -> Constant[T]: ...

    def deprefix_unit(self) -> Quantity[T]:
        '''remove the prefix of the unit.'''

    def to_basic_unit(self) -> Quantity[T]:
        '''transform unit to the combination of `_BASIC_SI` unit with 
        the same dimension.
        '''

    def simplify_unit(self) -> Quantity[T]:
        '''try if the complex unit can be simplified as u, u⁻¹, u², u⁻², 
        where u represents a single `_BASIC_SI` unit. 
        '''

    def remove_uncertainty(self) -> Quantity[T]:
        '''set uncertainty zero.'''

    def __eq__(self, other: Quantity[T]) -> bool: ...
    def __ne__(self, other: Quantity[T]) -> bool: ...
    def __gt__(self, other: Quantity[T]) -> bool: ...
    def __lt__(self, other: Quantity[T]) -> bool: ...
    def __ge__(self, other: Quantity[T]) -> bool: ...
    def __le__(self, other: Quantity[T]) -> bool: ...
    def __pos__(self) -> Quantity[T]: ...
    def __neg__(self) -> Quantity[T]: ...
    def __add__(self, other: T | Quantity[T]) -> Quantity[T]: ...
    def __sub__(self, other: T | Quantity[T]) -> Quantity[T]: ...
    def __mul__(self, other: float | T | Quantity[float] | Quantity[T]) -> Quantity[T]: ...
    def __matmul__(self, other: T | Quantity[T]) -> Quantity[T]: ...
    def __floordiv__(self, other: T | Quantity[T]) -> Quantity[T]: ...
    def __truediv__(self, other: float | T | Quantity[float] | Quantity[T]) -> Quantity[T]: ...
    def __pow__(self, other) -> Quantity[T]: ...
    def __iadd__(self, other: T | Quantity[T]) -> Quantity[T]: ...
    def __isub__(self, other: T | Quantity[T]) -> Quantity[T]: ...
    def __imul__(self, other: T | Quantity[T]) -> Quantity[T]: ...
    def __imatmul__(self, other: T | Quantity[T]) -> Quantity[T]: ...
    def __ifloordiv__(self, other: T | Quantity[T]) -> Quantity[T]: ...
    def __itruediv__(self, other: T | Quantity[T]) -> Quantity[T]: ...
    def __ipow__(self, other) -> Quantity[T]: ...
    def __radd__(self, other: T | Quantity[T]) -> Quantity[T]: ...
    def __rsub__(self, other: T | Quantity[T]) -> Quantity[T]: ...
    def __rmul__(self, other: float | T | Quantity[float] | Quantity[T]) -> Quantity[T]: ...
    def __rmatmul__(self, other: T | Quantity[T]) -> Quantity[T]: ...
    def __rfloordiv__(self, other: T | Quantity[T]) -> Quantity[T]: ...
    def __rtruediv__(self, other: float | T | Quantity[float] | Quantity[T]) -> Quantity[T]: ...
    def __rpow__(self, other) -> Quantity[T]: ...
    def nthroot(self, n: int) -> Quantity[T]: ...


def constant(quantity: Quantity[T]) -> Constant[T]: ...


class OneUnit(Constant[float]):
    '''Constant objects are just immutable Quantity objects.'''
    def __new__(cls, unit: str | Unit) -> OneUnit: ...
    @property
    def variable(self) -> Variable[float]: ...
    @property
    def value(self) -> float: ...
    @property
    def uncertainty(self) -> Zero: ...
    @property
    def relative_uncertainty(self) -> Zero: ...
    @property
    def unit(self) -> Unit: ...
    @property
    def dimension(self) -> Dimension: ...
    def __repr__(self) -> str: ...
    def __str__(self) -> str: ...
    def __format__(self, format_spec: str) -> str: ...
    def is_exact(self) -> bool: ...
    def is_dimensionless(self) -> bool: ...
    def copy(self) -> Constant: ...

    def to(self, new_unit: str | Unit, *,
           assertDimension=True) -> Constant[float]: ...

    def deprefix_unit(self) -> Quantity[float]:
        '''remove the prefix of the unit.'''

    def to_basic_unit(self) -> Quantity[float]:
        '''transform unit to the combination of `_BASIC_SI` unit with 
        the same dimension.
        '''

    def simplify_unit(self) -> Quantity[float]:
        '''try if the complex unit can be simplified as u, u⁻¹, u², u⁻², 
        where u represents a single `_BASIC_SI` unit. 
        '''

    def remove_uncertainty(self) -> Quantity[float]:
        '''set uncertainty zero.'''

    def __eq__(self, other: Quantity[float]) -> bool: ...
    def __ne__(self, other: Quantity[float]) -> bool: ...
    def __gt__(self, other: Quantity[float]) -> bool: ...
    def __lt__(self, other: Quantity[float]) -> bool: ...
    def __ge__(self, other: Quantity[float]) -> bool: ...
    def __le__(self, other: Quantity[float]) -> bool: ...
    def __pos__(self) -> Quantity[float]: ...
    def __neg__(self) -> Quantity[float]: ...
    def __add__(self, other: float | Quantity[float]) -> Quantity[float]: ...
    def __sub__(self, other: float | Quantity[float]) -> Quantity[float]: ...
    def __mul__(self, other: float | Quantity[float]) -> Quantity[float]: ...
    def __matmul__(self, other: float | Quantity[float]) -> Quantity[float]: ...
    def __floordiv__(self, other: float | Quantity[float]) -> Quantity[float]: ...
    def __truediv__(self, other: float | Quantity[float]) -> Quantity[float]: ...
    def __pow__(self, other) -> Quantity[float]: ...
    def __iadd__(self, other: float | Quantity[float]) -> Quantity[float]: ...
    def __isub__(self, other: float | Quantity[float]) -> Quantity[float]: ...
    def __imul__(self, other: float | Quantity[float]) -> Quantity[float]: ...
    def __imatmul__(self, other: float | Quantity[float]) -> Quantity[float]: ...
    def __ifloordiv__(self, other: float | Quantity[float]) -> Quantity[float]: ...
    def __itruediv__(self, other: float | Quantity[float]) -> Quantity[float]: ...
    def __ipow__(self, other) -> Quantity[float]: ...
    def __radd__(self, other: float | Quantity[float]) -> Quantity[float]: ...
    def __rsub__(self, other: float | Quantity[float]) -> Quantity[float]: ...
    def __rmul__(self, other: float| Quantity[float]) -> Quantity[float]: ...
    def __rmatmul__(self, other: float | Quantity[float]) -> Quantity[float]: ...
    def __rfloordiv__(self, other: float | Quantity[float]) -> Quantity[float]: ...
    def __rtruediv__(self, other: float | Quantity[float]) -> Quantity[float]: ...
    def __rpow__(self, other) -> Quantity[float]: ...
    def nthroot(self, n: int) -> Quantity[float]: ...